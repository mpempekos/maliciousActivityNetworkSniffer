...
//            	gui.ReportTerminalOperations.addReport("> SELECT * FROM statistics WHERE maliciousPattern='"+filter+"'");
	...
	public static boolean register(String username, String password, AvailableNodes nodes) {
		...
		try {
			...
			String query_2 = " insert into android_clients (username, password)"
					+ " values (?, ?);";
			st = conn.prepareStatement(query_2);
			st.setString(1, username);
	...
	public static boolean checkMobileExistance(String username, String password) {
    	DBCreation sql = DBCreation.getInstance();
        Connection conn;
    	PreparedStatement st;
    	conn = sql.connect();
		...
		try {
			...
			String query = "select * from android_clients where username = ? AND password = ? ";
			st = conn.prepareStatement(query);
			st.setString(1, username);
			st.setString(2, password);
			res = st.executeQuery();
	...
	protected static void deleteNodeStatistics(String nodeID) {
    	DBCreation sql = DBCreation.getInstance();
        Connection conn;
    	PreparedStatement st;
    	conn = sql.connect();
		...
		try {
    		st = conn.prepareStatement("DELETE FROM statistics WHERE nodeID=?");
    		st.setString(1, nodeID);
            st.executeUpdate();
            st.clearParameters();
    	}
	...
	public static void deleteNode(String nodeID) {
		...
		try {
			...
			deleteNodeStatistics(nodeID);
			
			
			st.clearParameters();
    		st = conn.prepareStatement("DELETE FROM clients WHERE clientID=?");
    		st.setString(1, nodeID);
            st.executeUpdate();
            st.clearParameters();
    	}
	...
	public static String getMaliciousByID(int id) {
		String maliciousPattern = new String();
		String maliciousIP = new String();
    	...
    	try {
    		...
    		st = conn.prepareStatement("select * from maliciousPatterns where patternID = ? ");
    		st.setInt(1, id);
    		res = st.executeQuery();
        
    		if (!res.next())
    			return "(unknown)";
    		maliciousPattern = res.getString("maliciousStringPattern");
	    	maliciousIP = res.getString("maliciousIP");
